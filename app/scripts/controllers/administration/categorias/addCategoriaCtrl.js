angular.module('fotosApp')
  .controller('addCategoriaCtrl', function (userService,$rootScope,alertsService,$localStorage,$scope,item,$uibModalInstance,toastr,FileUploader,generalService,categoriaService) {
    ...
    $scope.save = function() {
      if (typeof item == 'undefined') {
        ...
        categoriaService.addCategoria($scope.item).then(function(r){
    ...
    $scope.setKey = function(){
      var strg = $scope.item.nombre;
      $scope.item.key = strg.replace(/\s/g,"_").toLowerCase();
    }
